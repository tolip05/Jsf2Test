...
public class PersonSelectByCriteriaAnyCityBuilder{

private static String sqlString = "select * from (select COUNT(*) OVER(ORDER BY 0) rows_count, a.*, ROW_NUMBER() OVER (ORDER BY 0) rnum from ( select person_id, first_name, last_name, uid, address_id, gender from registers.person where upper(first_name) = coalesce(?, upper(first_name)) and upper(last_name) = coalesce(?, upper(last_name)) and upper(uid) = coalesce(?, upper(uid)) and address_id = any(?) and upper(gender) = coalesce(?, upper(gender)) ) a ) b where rnum  >= coalesce(?,rnum) and rnum <= coalesce(?,rnum) ";
...
public static interface Param4 {
		Param5 setParam4_AddressId(java.util.List<java.lang.Long> addressId);
...
private java.util.List<com.cc.pojo.Param> params = new java.util.ArrayList<com.cc.pojo.Param>();
	...
	public Param5 setParam4_AddressId(java.util.List<java.lang.Long> addressId){
		params.add(new com.cc.pojo.Param(com.cc.annotation.Parameters.ParamTypes.LongArray, addressId));
	return this;
	}
